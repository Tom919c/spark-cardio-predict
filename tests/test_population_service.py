...
def test_population_dashboard_and_follow_up_use_cached_dataframe(tmp_path: Path):
    dataset = tmp_path / "population.csv"
    pd.DataFrame(
        [
            [70, "Jinjiang", "60-74", 31.2, 1, 1, 3, 2, 0, 1, 1],
            [45, "Qingyang", "40-59", 23.5, 0, 0, 1, 0, 1, 0, 0],
            [66, "Jinjiang", "60-74", 27.1, 1, 0, 2, 1, 0, 0, 1],
        ],
...
def test_population_service_reports_missing_file(tmp_path: Path):
    service = PopulationService({"POPULATION_DATASET_PATH": str(tmp_path / "missing.csv")})

    try:
        service.dashboard()
    except FileNotFoundError as exc:
        assert "Population dataset is missing" in str(exc)
    else:
