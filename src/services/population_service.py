...
class PopulationService:
    ...
    def _dataframe(self):
        if not self.population_path.exists():
            raise FileNotFoundError(
                "Population dataset is missing. Run scripts/generate_chengdu_health_data.py first."
            )
    ...
    def _risk_factors(self, dataframe):
        factors = {
            "hypertension": dataframe["hypertension"].mean(),
            "diabetes": dataframe["diabetes"].mean(),
            "high_cholesterol": (dataframe["cholesterol"] >= 2).mean(),
            "current_smoker": (dataframe["smoker"] == 2).mean(),
            "overweight_or_obese": (dataframe["bmi"] >= 25).mean(),
            "inactive": (dataframe["exercise"] == 0).mean(),
        }
        return [
